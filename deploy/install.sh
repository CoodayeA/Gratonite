...
show_sharing_guide() {
  echo ""
  echo -e "    ${BOLD}No worries — let's figure out the best way for you.${NC}"
  echo ""
  echo "    First, a quick explanation: when you visit a website like"
  echo "    google.com, your browser uses that name (called a \"domain\")"
  echo "    to find the right computer on the internet. Your Gratonite"
  echo "    server needs a name like that too, so your friends can find it."
  echo ""
  echo "    The good news? ${BOLD}You don't have to pay for one.${NC}"
  echo ""
  echo -e "  ${DIM}  ─────────────────────────────────────────────────────${NC}"
  echo ""
  echo -e "    ${GREEN}${BOLD}Option A: Get a free name with DuckDNS${NC}  ${GREEN}← recommended${NC}"
  echo ""
  ...
  echo "    for your server. No credit card, no catch. It's been"
  echo "    around since 2013 and is trusted by millions."
  echo ""
  echo "    Here's how it works:"
  echo "    • You pick a name (like \"mychat\" or \"ourfriends\")"
  echo "    • DuckDNS points that name at your computer's IP address"
  echo "    • Anyone who types mychat.duckdns.org in their browser"
  echo "      gets sent straight to your Gratonite server"
  echo ""
  echo "    It takes about 2 minutes:"
  echo -e "    1. Open ${BOLD}duckdns.org${NC} in your browser"
  echo "    2. Click one of the sign-in buttons (GitHub, Google, etc.)"
  echo "    3. Type a name you want in the box and click \"add domain\""
  echo "    4. That's it — come back here and type that name in"
  echo ""
  echo -e "  ${DIM}  ─────────────────────────────────────────────────────${NC}"
  echo ""
  echo -e "    ${CYAN}${BOLD}Option B: Cloudflare Tunnel${NC}  (zero configuration)"
  echo ""
  echo "    If you don't want any name at all, Cloudflare Tunnel"
  echo "    creates a secret passageway from your computer to the"
  echo "    internet. It gives you a random link you can share."
  echo ""
  echo "    It's great because it works even if you're behind a"
  echo "    router or firewall — no settings to change anywhere."
  echo ""
  echo "    We'll install Gratonite first, then you just run one"
  echo "    command to create the tunnel."
  echo ""
  echo -e "  ${DIM}  ─────────────────────────────────────────────────────${NC}"
  echo ""
  echo -e "    ${PURPLE}${BOLD}Option C: Tailscale${NC}  (private — invite only)"
  echo ""
  echo "    Tailscale creates a private network that only people"
  echo "    you invite can join. It's invisible to the rest of the"
  echo "    internet — like a secret club."
  echo ""
  echo "    Everyone (you + friends) installs the Tailscale app,"
  echo "    and then they can reach your Gratonite at a special"
  echo "    private address. Free for up to 100 people."
  echo ""
  echo "    Best for: small friend groups who want maximum privacy."
  echo ""
  echo -e "  ${DIM}  ─────────────────────────────────────────────────────${NC}"
  echo ""
  echo -e "    ${BOLD}What sounds right for you?${NC}"
  echo ""
  echo -e "      ${BOLD}a)${NC} I'll grab a free DuckDNS name right now (2 min)"
  echo -e "      ${BOLD}b)${NC} Cloudflare Tunnel sounds good — set up locally first"
  echo -e "      ${BOLD}c)${NC} Tailscale for my friend group — set up locally first"
  echo ""
  prompt -rp "    Pick one [a/b/c]: " SHARING_CHOICE

  case "${SHARING_CHOICE:-a}" in
    a|A)
      echo ""
      echo -e "    ${GREEN}${BOLD}Great choice!${NC}"
      echo ""
      echo "    Go set up your DuckDNS name now — it's quick:"
      echo ""
      echo -e "      1. Open ${BOLD}https://duckdns.org${NC} in your browser"
      echo "      2. Sign in with GitHub, Google, or another account"
      echo "      3. Type a name you like and click \"add domain\""
    ...
    b|B)
      echo ""
      echo -e "    ${CYAN}${BOLD}Good pick!${NC} We'll install Gratonite in local mode first."
      echo ""
      echo "    After the install finishes, create your tunnel by running:"
      echo ""
      echo -e "      ${BOLD}cloudflared tunnel --url https://localhost:8443${NC}"
      echo ""
      echo "    (If you don't have cloudflared yet, install it first:"
      echo -e "     Mac: ${BOLD}brew install cloudflared${NC}"
      echo -e "     Linux: ${BOLD}sudo apt install cloudflared${NC} or see ${BOLD}developers.cloudflare.com/cloudflare-one${NC})"
      echo ""
      echo "    Cloudflare will print a URL like"
      echo -e "    ${CYAN}https://abc-xyz-123.trycloudflare.com${NC}"
      echo "    Share that with your friends and they can join!"
      echo ""
      MODE="local"
      ;;
    c|C)
      echo ""
      echo -e "    ${PURPLE}${BOLD}Nice — ultra private!${NC} We'll install Gratonite in local mode first."
      echo ""
      echo "    After the install finishes, set up Tailscale:"
      echo ""
      echo -e "    1. Install Tailscale: ${BOLD}https://tailscale.com/download${NC}"
      echo "    2. Run: tailscale up"
      echo "    3. Your friends install Tailscale too and join your network"
      echo "    4. Share your Tailscale IP (shown in the Tailscale app)"
      echo "    5. They visit https://YOUR_TAILSCALE_IP:8443"
      echo ""
      echo "    Only people in your Tailscale network can connect."
      echo "    Everyone else can't even see your server exists."
      echo ""
      MODE="local"
      ;;
    *)
