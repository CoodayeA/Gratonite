...
const InviteModal = ({ onClose, guildId }: { onClose: () => void; guildId: string | null }) => {
    ...
    return (
        <div className="modal-backdrop" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={onClose}>
            <div role="dialog" aria-modal="true" aria-label="Invite to portal" className="login-box glass-panel" style={{ width: 'min(480px, 95vw)', padding: '32px', position: 'relative', maxHeight: '90vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
