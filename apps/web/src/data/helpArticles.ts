...
export const ARTICLES: Article[] = [
  ...
  },
  {
    ...
    description: 'Browse, install, and configure bots for your guild.',
    category: 'Bots & Integrations',
    body: [
      'Open the Bot Store from guild settings or the main navigation to browse available bots.',
      'Use categories and search to find bots for moderation, music, leveling, utilities, and more.',
      'Click "Add to Server" and choose which guild to install the bot in — you need the "Manage Server" permission.',
  ...
  },
  {
    ...
    category: 'Bots & Integrations',
    body: [
      'Navigate to Guild Settings > Integrations > Webhooks and click "New Webhook".',
  ...
  },
  {
    ...
    body: [
      ...
      'Earn coins by participating in guilds: chatting, reacting, attending events, and helping others.',
  ...
  },
  {
    ...
    body: [
      ...
      'Equipped items are reflected on your profile card and everywhere your avatar appears across Gratonite — including guild member lists, chat messages, and DMs.',
  ...
  },
  {
    ...
    body: [
      ...
      'Frames are visible in the guild member list, next to your messages in any text channel, on your profile card when someone clicks your name, and in direct message conversations.',
  ...
  },
  {
    ...
    body: [
      ...
      'Nameplates are displayed in all text channels next to your messages, in the guild member list, and on your profile card. They apply to your display name as set in your profile — they do not affect your username handle.',
      'To equip a nameplate, open Inventory, go to the Nameplates tab, choose the nameplate you want, and click "Equip". Your name will update immediately across all guilds and channels without requiring any action from other users.',
...

  {
    id: 'bots-overview',
    title: 'Bots — Overview',
    description: 'What bots are and how they enhance your Gratonite guilds.',
    category: 'Bots & Integrations',
    body: [
      'Bots are automated programs that connect to Gratonite and add functionality inside your guilds — things like custom commands, automated moderation, leveling systems, polls, and third-party service notifications.',
      ...
      'Regardless of type, bots can send and edit messages, create and manage threads, assign or remove roles, listen for specific events (a member joining, a reaction being added, a message being posted), and respond to slash commands typed by guild members.',
      'To find bots ready to install, visit the Bot Store at /bot-store. Each listing includes a description, a list of permissions the bot requires, and reviews from other guild admins who have installed it.',
      'Guild members interact with bots primarily through slash commands (typed with a "/" prefix), keyword triggers (the bot watches for specific words in messages), and event listeners (the bot takes action automatically when a guild event occurs, such as posting a welcome message when a new member joins).',
    ],
  },
  {
    id: 'adding-bot-to-server',
    title: 'Adding a Bot to Your Guild',
    description: 'Step-by-step guide for guild admins to install and configure bots.',
    category: 'Bots & Integrations',
    body: [
      'You must have the "Manage Server" permission on the target guild to install bots. If you are not the guild owner, ask an admin to grant you this permission or have them perform the installation.',
      ...
      'On the bot\'s detail page, click "Add to Server". A modal will appear listing every guild where you have the "Manage Server" permission. Select the guild you want to install the bot into and click "Next".',
      'Review the list of permissions the bot is requesting — for example, Read Messages, Send Messages, Manage Roles, or Kick Members. These permissions define what the bot is allowed to do in your guild. Click "Authorize" to confirm and complete the installation.',
      'After installation, the bot appears in your guild\'s member list with a "Bot" badge next to its name. It is now connected and listening for events in your guild.',
      'To configure the bot\'s access, go to Guild Settings > Roles and adjust the bot\'s role permissions. You can also go to individual channel settings and use permission overrides to restrict or expand the bot\'s access per channel.',
      ...
      'To remove a bot, go to Guild Settings > Integrations, find the bot in the installed list, and click "Remove". All data the bot stored for your guild may be deleted — check the bot\'s documentation for details.',
    ],
  },
  {
    ...
    category: 'Bots & Integrations',
    body: [
      'A Webhook bot is a self-hosted HTTP server that you build and run. Rather than maintaining a persistent connection to Gratonite, your server receives events as HTTP POST requests that Gratonite sends whenever something happens in a guild where your bot is installed.',
  ...
  {
    id: 'bot-migration',
    title: 'Migrating an Existing Bot to Gratonite',
    description: 'How to adapt your existing webhook-based bot to work on Gratonite using the bridge adapter.',
    category: 'Bots & Integrations',
    body: [
      'Gratonite\'s webhook-based bot system is fundamentally simpler than gateway-based systems: instead of opening a persistent WebSocket connection and maintaining a heartbeat, your bot server just receives HTTP POST requests and responds with action JSON. This means less infrastructure to manage and no reconnection logic.',
      'The key architectural difference is that some platforms stream all events to your bot continuously over a WebSocket, whereas Gratonite pushes only relevant events to your registered webhookUrl as individual HTTP requests. Your bot does not need to be "always listening" — it just needs to be able to handle incoming POST requests.',
      'Most event and field names map directly with small naming differences. The message_create event is the standard entry point. The author.id field identifies the message sender. The top-level guildId identifies the community. Refer to the full mapping table in the Bot Builder documentation.',
      'To accelerate migration, download the official Gratonite Bridge Adapter from Bot Builder > Documentation > Bridge Adapter. This TypeScript package wraps Gratonite\'s webhook event format in a compatible structure, so your existing event handler functions require minimal or no changes to run.',
      'Install the adapter with npm install @gratonite/bot-bridge, then replace your existing client initialization with the GratoniteClient provided by the adapter, passing in your webhookSecret and apiToken. The adapter exposes a client.on("messageCreate", handler) API.',
      'The majority of features work out of the box after migration: text commands, slash command responses, role assignment and removal, and message sending and editing are all fully supported. Voice and audio playback requires a separate integration with Gratonite\'s LiveKit-based voice infrastructure — consult the LiveKit Integration guide for details.',
      'To test your migrated bot, register a webhook URL pointing to your adapter server in the Bot Builder, install the bot on a private test guild, and send a command. Check the Bot Builder > Logs tab to inspect the raw event payloads Gratonite sent and the action responses your server returned, which makes debugging straightforward.',
    ],
  },
...

  {
    ...
    body: [
      ...
      'Threads auto-archive after a configurable period of inactivity. Guild admins can set the auto-archive duration in channel settings.',
  ...
  },
  {
    ...
    body: [
      ...
      'Use Ctrl+K (or Cmd+K on Mac) to open the command palette for quick navigation to channels, guilds, and DMs.',
  ...
  },
  {
    ...
    body: [
      ...
      'Click your own reaction again to remove it. Guild moderators can remove reactions from any message.',
      'Guilds can restrict which roles are allowed to add reactions in specific channels via channel permission settings.',
    ],
  },
  {
    id: 'drafts-scheduled',
  ...
  },
  {
    ...
    body: [
      ...
      'Bookmarks are private — only you can see your saved messages. They are not visible to other users or guild admins.',
      'Organize bookmarks by searching or filtering by guild and channel to quickly find the message you need.',
  ...
  },
  {
    ...
    body: [
      ...
      'DM conversations support all the same features as guild channels — file uploads, reactions, embeds, and message editing.',
  ...
  },
  {
    ...
    body: [
      ...
      'Clear all unread notifications for a guild by right-clicking its icon and selecting "Mark as Read".',
  ...
  },
  {
    ...
    body: [
      ...
      'Data exports include your messages, profile information, guild memberships, uploads, and activity history in a machine-readable format.',
  ...
  {
    id: 'setting-your-status',
    title: 'Setting Your Status',
    description: 'Choose between Online, Idle, Do Not Disturb, and Invisible.',
    category: 'Getting Started',
    body: [
      'Click your avatar at the bottom-left of the sidebar to open the status picker.',
      'Online (green dot) — You\'re active and available for messages and calls.',
      'Idle (amber dot) — You\'re away or not actively using Gratonite. Others can still message you normally.',
      'Do Not Disturb (red dot) — You won\'t receive notification sounds or popup alerts. Messages still arrive, but silently.',
      'Invisible (gray dot) — You appear completely offline to everyone else, but you can still browse, read, and send messages normally.',
      'Your chosen status is saved automatically and restored when you log back in.',
    ],
  },
  {
    id: 'automatic-idle',
    title: 'Automatic Idle & Going Offline',
    description: 'How Gratonite detects inactivity and handles disconnections.',
    category: 'Getting Started',
    body: [
      'If you\'re set to Online and haven\'t interacted with Gratonite for 15 minutes, your status automatically switches to Idle.',
      'As soon as you move your mouse, type, or scroll, your status switches back to Online instantly.',
      'Automatic idle does NOT override Do Not Disturb or Invisible — those stay until you change them manually.',
      'On the desktop app, Gratonite also detects when your computer goes to sleep or the screensaver activates, and marks you as Idle.',
      'When you close Gratonite or lose your internet connection, you\'ll appear Offline to others after about 30 seconds. This short grace period prevents your status from flickering during brief disconnections like switching Wi-Fi networks.',
    ],
  },
  {
    id: 'rich-presence-activity',
    title: 'Activity Status & Rich Presence',
    description: 'Show what you\'re up to — like playing a game or listening to music.',
    category: 'Getting Started',
    body: [
      'Set an activity status to let friends and guild members know what you\'re doing — for example, "Playing Minecraft" or "Listening to Spotify."',
      'Your activity appears on your profile card, in the member list sidebar, and on your profile popover when someone clicks your name.',
      'Activity statuses are temporary — they clear automatically when you go offline or close the app.',
      'If you\'re set to Invisible, your activity status is hidden from other users for privacy.',
      'On the desktop app, Gratonite can detect running applications and show them as your activity automatically (enable in Settings > Connections).',
    ],
  },
  {
    id: 'seeing-others-status',
    title: 'Seeing Other Users\' Status',
    description: 'Where to find presence indicators throughout Gratonite.',
    category: 'Getting Started',
    body: [
      'Status dots appear on user avatars throughout Gratonite: green for Online, amber for Idle, red for Do Not Disturb, and gray for Offline.',
      'Check the member list (right sidebar in any channel) to see who\'s online in a guild.',
      'Your DM conversations list shows status dots next to each person so you can see who\'s available before messaging.',
      'The Friends page shows all your friends grouped by their current status, making it easy to find who\'s around.',
      'Click any username or avatar to open their profile popover, which shows their current status and activity.',
    ],
  },
  {
    id: 'presence-privacy',
    title: 'Status & Presence Privacy',
    description: 'How your status information is shared and protected.',
    category: 'Privacy & Safety',
    body: [
      'Your Invisible status is never revealed to other users — they always see you as Offline. There is no way for anyone to tell the difference.',
      'Your status is only visible to members of guilds you belong to and your friends list. Strangers cannot see your presence.',
      'Activity status (like what game you\'re playing) is automatically hidden when you\'re set to Invisible.',
      'You can disable rich presence entirely in Settings > Privacy if you don\'t want others to see your activity at all.',
      'Gratonite does not store a history of your status changes — only your current status is tracked.',
    ],
  },
];
