                                                        ...
                                                        <div style={{ overflow: 'hidden', flex: 1 }}>
                                                            ...
                                                            {dm.lastMessage?.content ? (
                                                                <span style={{ fontSize: '11px', color: gdmHasUnread ? 'var(--text-secondary)' : 'var(--text-muted)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', display: 'block', fontWeight: gdmHasUnread ? 500 : undefined }}>
                                                                    {(dm.lastMessage as any).authorUsername ? <span style={{ fontWeight: 600, marginRight: '2px', color: gdmHasUnread ? 'var(--text-primary)' : 'var(--text-secondary)' }}>{(dm.lastMessage as any).authorUsername.split(' ')[0]}:</span> : null}{(dm.lastMessage.content as string).replace(/!\[.*?\]\(.*?\)/g, '📎 Attachment').replace(/```[\s\S]*?```/g, '📝 Code').substring(0, 40)}{(dm.lastMessage.content as string).length > 40 ? '…' : ''}
                                                                </span>
                                                            ) : memberCount > 0 ? (
