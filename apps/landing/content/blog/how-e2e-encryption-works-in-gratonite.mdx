...
description: "A plain-English technical look at how Gratonite encrypts DMs and group conversations, from key generation to message delivery."
date: "2026-03-09"
...
We use two boring, trusted cryptographic building blocks, both provided by the **Web Crypto API** already built into modern browsers:
...
We picked standard crypto on purpose. ECDH P-256 and AES-GCM are widely audited, already available in the browser, and do not require pulling in extra crypto dependencies just to feel clever.
...
Your client performs the ECDH computation: your private key + their public key = a shared **AES-GCM 256-bit symmetric key**. The other user's client does the same computation and arrives at the **exact same shared key**. That is the trick Diffie-Hellman gives you.
...
The current system gives strong confidentiality against server compromise and passive eavesdropping. It does not protect against an attacker who both steals your private key and has recorded your ciphertext — that is the gap forward secrecy would close.
