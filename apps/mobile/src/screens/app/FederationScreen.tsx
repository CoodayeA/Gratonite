...
export default function FederationScreen({ navigation }: Props) {
  ...
  const renderGuild = ({ item }: { item: RemoteGuild }) => (
    <PressableScale style={[styles.guildCard, { backgroundColor: colors.card, borderRadius: borderRadius.lg }]}>
      <View style={styles.guildHeader}>
        <View style={[styles.guildIcon, { backgroundColor: colors.primary + '20', borderRadius: borderRadius.md }]}>
          <Text style={{ color: colors.primary, fontWeight: '700', fontSize: fontSize.lg }}>
            {item.name.charAt(0).toUpperCase()}
          </Text>
        </View>
        <View style={{ flex: 1, marginLeft: 12 }}>
          <Text style={{ color: colors.text, fontWeight: '700', fontSize: fontSize.md }} numberOfLines={1}>
            {item.name}
          </Text>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 2 }}>
            <View style={[styles.badge, { backgroundColor: trustColor(item.instance.trustLevel) + '20' }]}>
              <Text style={{ color: trustColor(item.instance.trustLevel), fontSize: 10, fontWeight: '600' }}>
                {new URL(item.instance.baseUrl).hostname}
              </Text>
            </View>
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={[styles.header, { padding: spacing.md }]}>
        ...
        <Text style={{ color: colors.text, fontWeight: '700', fontSize: fontSize.lg, marginLeft: 8 }}>
          Federation
        </Text>
      </View>
